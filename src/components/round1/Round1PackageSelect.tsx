  ...
  return (
    <div className="relative min-h-[calc(100vh-68px)] p-6 md:p-8 lg:p-12 flex flex-col justify-between bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
          ...
          <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs md:text-sm font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300">
                  Phần Thi Giao Lưu Cùng Khán Giả
                </span>
              </div>
              <span className="text-[11px] text-amber-800 dark:text-amber-400/80 bg-amber-100 dark:bg-amber-950/50 px-2.5 py-0.5 rounded-full border border-amber-300 dark:border-amber-500/30 font-semibold">
            ...
            <button
              ...
              className={`w-full group rounded-3xl p-5 md:p-6 text-left transition-all duration-300 border-2 active:scale-98 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 ${
                audiencePackage.status === 'completed'
                  ? 'bg-gradient-to-r from-emerald-50 via-white to-emerald-50 dark:from-slate-900 dark:via-emerald-950/40 dark:to-slate-900 border-emerald-400 dark:border-emerald-500/40 hover:border-emerald-500'
                  : 'bg-gradient-to-r from-amber-50/80 via-white to-amber-50/80 dark:from-slate-900 dark:via-amber-950/30 dark:to-slate-900 border-amber-400 dark:border-amber-500/50 hover:border-amber-500 dark:hover:border-amber-400 shadow-lg dark:hover:shadow-2xl dark:hover:shadow-amber-950/50'
              }`}
            >
              <div className="flex items-center gap-4 md:gap-5">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-slate-950 shadow-xl shadow-amber-950/20 dark:shadow-amber-950/60 font-black text-2xl md:text-3xl shrink-0 group-hover:scale-105 transition-transform">
                  <Gift className="w-9 h-9 text-slate-950" />
                </div>
                <div>
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-400/20 text-amber-800 dark:text-amber-300 text-xs font-bold border border-amber-300 dark:border-amber-400/30 mb-1.5">
                    <Users className="w-3.5 h-3.5" />
                    <span>GÓI SỐ 11 • GIAO LƯU KHÁN GIẢ</span>
                  </div>
                  <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-300 transition-colors">
                    {audiencePackage.title}
                  </h3>
                  <p className="text-xs md:text-sm text-slate-700 dark:text-slate-300 mt-1 max-w-2xl font-medium">
                    Gồm <span className="text-amber-600 dark:text-amber-400 font-bold">{audiencePackage.questions.length} câu hỏi trắc nghiệm</span> về Chuyển đổi số, VNeID, Dịch vụ công, Tam Hải dành riêng cho bà con nhân dân và cổ động viên. Trả lời đúng nhận quà từ BTC!
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 shrink-0 self-end md:self-auto">
                {audiencePackage.status === 'completed' ? (
                  <div className="px-4 py-2 rounded-xl bg-emerald-100 dark:bg-emerald-950/90 border border-emerald-400 dark:border-emerald-500/50 text-emerald-800 dark:text-emerald-300 text-xs font-bold flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4" />
                    <span>ĐÃ THI GIAO LƯU</span>
                  </div>
                ) : (
                  <div className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs md:text-sm font-black flex items-center gap-2 shadow-lg shadow-amber-950/20 dark:shadow-amber-950/40 group-hover:scale-105 transition">
      ...
      <div className="relative z-10 max-w-4xl mx-auto w-full text-center text-xs md:text-sm text-slate-600 dark:text-slate-400 p-3 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 shadow-sm">
        <span className="text-cyan-700 dark:text-cyan-400 font-bold">Lưu ý máy chiếu:</span> Chức năng <strong className="text-cyan-800 dark:text-cyan-300">Bốc Thăm Ngẫu Nhiên</strong> chỉ áp dụng cho 10 gói của các đội thi (Gói 1 - 10). Gói 11 dành riêng cho tương tác khán giả.
      </div>
